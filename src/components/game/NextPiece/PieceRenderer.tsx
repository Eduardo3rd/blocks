...
export const PieceRenderer: React.FC<PieceRendererProps> = ({ piece, scale = 1 }) => {
  ...
  const width = piece.shape[0].length * blockSize + 2 * padding;
  const height = piece.shape.length * blockSize + 2 * padding;
          ...
          return (
            <Block key={`${x}-${y}`} color={piece.color}>
              ...
              <rect
                ...
                x={x * blockSize + padding + 2 * scale}
                y={y * blockSize + padding + 2 * scale}
                width={blockSize - 4 * scale}
                height={blockSize - 4 * scale}
              />
            </Block>
          );
        })
